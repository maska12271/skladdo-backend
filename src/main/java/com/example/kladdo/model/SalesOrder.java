...
public class SalesOrder {
    ...
    /**
     * Sum of per-line tax. Kept separate so {@link #totalAmount} stays net (subtotal + delivery) and
     * existing revenue analytics are unaffected; the tax-inclusive total is {@code totalAmount + taxAmount}.
     * Nullable for orders created before tax tracking (treated as zero).
     */
    private BigDecimal taxAmount = BigDecimal.ZERO;

    /** Net total: {@link #subtotalAmount} (after line discounts) plus {@link #deliveryPrice}. Excludes tax. */
    @Column(nullable = false)
    ...
    // --- Optional per-order payment terms (override the company settings when set) ----------------
    // All nullable: a null value means "fall back to the company-wide CompanySettings value" when the
    // invoice is generated.

    /** Override for the invoice payment term in days. Null = use the company default. */
    private Integer paymentTermDays;

    /** Override for the late-payment penalty percentage. Null = use the company default. */
    private BigDecimal penaltyPercent;

    /** Override for how the penalty accrues. Null = use the company default. */
    @Enumerated(EnumType.STRING)
    ...
    /**
     * Source warehouse from which goods are shipped for this order. Eagerly fetched (like
     * {@link #client}) so it serializes when the entity is returned directly with
